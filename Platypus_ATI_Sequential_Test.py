...
for drive in drives.drive_list_single_path:
    ...
    ati_file_name[drive] = prodid+'_sequential_'+sp_position+'.bin'
...
for prodid in unique_product_ids:
    ...
    for z in range(test_config_dict["number_of_ati_zones"]):
        ati_zone_starting_lbas.append( z*(lba_max+1)/test_config_dict["number_of_ati_zones"] + track_size)
    with open(prodid+'_sequential_'+sp_position+'.bin','wb') as f_bin_out:
        with open(prodid+'_sequential_'+sp_position+'.csv','w') as f_csv_out:
            for start_lba in ati_zone_starting_lbas:
                for lba in range( start_lba, start_lba + track_size * test_config_dict["tracks_per_ati_zone"], track_size * 2 ):
    ...
    # transfer to SP
    FileTransfer(scheduler, './'+prodid+'_sequential_'+sp_position+'.bin', '/platypus', 'local', {'Protocol':'FTP','Mode':'AUTO'})

#Dump the drive logs
drives.get_logs()

for loop in range(4): #this loops ATI test 4 times - 120,000 ATI writes total   
    #
